...
from voice_assistant.prosa import Prosa
...
def transcribe_audio(model, api_key, audio_file_path, local_model_path=None):
    ...
    try:
        if model == 'openai':
            ...
            return _transcribe_with_prosa(api_key,audio_file_path)
...
def _transcribe_with_prosa(api_key, audio_file_path):
    prosa = Prosa.STT(api_key)
    transcription = prosa.create_transcription(audio_file_path)
